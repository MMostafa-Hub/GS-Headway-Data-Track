...
def add_weekly_seasonality(data, seasonality, season_type):
    ...
    if seasonality == "exist":  # Weekly Seasonality
        ...
        seasonal_component = (
            np.zeros(len(data)) if season_type == "additive" else np.ones(len(data))
        )
...
def add_trend(data, trend, data_size, data_type):
    ...
    if trend == "exist":
        ...
        trend_component = (
            np.linspace(0, data_size / 30 * slope, len(data))
            if slope == 1
            else np.linspace(-1 * data_size / 30, 0, len(data))
        )
    else:  # No Trend
        trend_component = (
            np.zeros(len(data)) if data_type == "additive" else np.ones(len(data))
        )
...
def add_noise(data, noise_level):
    ...
    for i in range(len(data)):
        noise[i] = (
            np.random.normal(0, abs(data[i]) * noise_level) if noise_level > 0 else 0
        )
                        ...
                        for percentage_outliers in percentage_outliers_options:
                            for data_type in data_types:
                                for _ in range(16):
                                    ...
                                    # Generate time index
                                    date_rng = generate_time_series(
                                        start_date,
                                        start_date + timedelta(days=data_size),
                                        freq,
                                    )
                                    # Create components
                                    daily_seasonal_component = add_daily_seasonality(
                                        date_rng,
                                        daily_seasonality,
                                        season_type=data_type,
                                    )
                                    weekly_seasonal_component = add_weekly_seasonality(
                                        date_rng,
                                        weekly_seasonality,
                                        season_type=data_type,
                                    )
                                    trend_component = add_trend(
                                        date_rng,
                                        trend,
                                        data_size=data_size,
                                        data_type=data_type,
                                    ...
                                    cyclic_period = "exist"
                                    cyclic_component = add_cycles(
                                        date_rng, cyclic_period, season_type=data_type
                                    )

                                    # Combine components and add missing values and outliers
                                    if data_type == "multiplicative":
                                        data = (
                                            daily_seasonal_component
                                            * weekly_seasonal_component
                                            * trend_component
                                            * cyclic_component
                                        )
                                    else:
                                    ...
                                    data, anomaly = add_outliers(
                                        data, percentage_outliers
                                    )
                                    ...
                                    df.to_csv(
                                        "sample_datasets/" + str(counter) + ".csv",
                                        encoding="utf-8",
                                        index=False,
                                    )
